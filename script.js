...
let networkViewCounts = {}; // { networkId: count }
...
function applyFilters(feeds) {
    ...
    if (selectedNetworkId) {
        // Bu örnek, ağ bazlı filtreleme için bir mantık içermez çünkü DATA_FEEDS
        // doğrudan ağla ilişkilendirilmemiş. Ağ detay modalında filtreleme yapılabilir.
        console.warn("Ağ filtresi uygulanamadı: DATA_FEEDS ağla doğrudan ilişkilendirilmemiş.");
    }
...
function loadNetworkList(page = 1) {
    ...
    const totalNetworks = sortedNetworks.length;
    const totalPages = Math.ceil(totalNetworks / NETWORKS_PER_PAGE);

    if (page < 1) page = 1;
    if (page > totalPages) page = totalPages;
    currentNetworkPage = page;

    const startIndex = (page - 1) * NETWORKS_PER_PAGE;
    const endIndex = Math.min(startIndex + NETWORKS_PER_PAGE, totalNetworks);
    const networksToShow = sortedNetworks.slice(startIndex, endIndex);

    networkListDiv.innerHTML = networksToShow.map(network => `
    ...
    if (prevBtn) prevBtn.addEventListener('click', () => loadNetworkList(currentNetworkPage - 1));
    if (nextBtn) nextBtn.addEventListener('click', () => loadNetworkList(currentNetworkPage + 1));
...
function showNetworkDetailsModal(network) {
    ...
    // Ağ bazlı veri akışlarını filtrele (örnek: adresle eşleşenleri bul)
    // Bu örnek, tüm veri akışlarını gösterir. Gerçek uygulamada ağla ilişkili
    // veri akışlarını belirlemek için daha karmaşık bir mantık gerekir.
    const networkFeeds = DATA_FEEDS; // Şimdilik tüm akışlar
...
function loadFeedList(page = 1) {
    ...
    const totalPages = Math.ceil(totalFeeds / FEEDS_PER_PAGE);
    ...
    const feedsToShow = feedsToDisplay.slice(startIndex, endIndex);

    feedListDiv.innerHTML = feedsToShow.map(feed => `
        <div class="feed-item">
    ...
    feedPaginationDiv.innerHTML = `
        <button class="pagination-btn" id="prevFeedPageBtn" ${page === 1 || totalPages === 0 ? 'disabled' : ''}>
            <i class="fas fa-chevron-left"></i> Prev
        </button>
        <span class="pagination-info">Page ${page} of ${totalPages || 1}</span>
        <button class="pagination-btn" id="nextFeedPageBtn" ${page === totalPages || totalPages === 0 ? 'disabled' : ''}>
            Next <i class="fas fa-chevron-right"></i>
        </button>
    `;

    const prevFeedBtn = document.getElementById('prevFeedPageBtn');
    const nextFeedBtn = document.getElementById('nextFeedPageBtn');
    if (prevFeedBtn) prevFeedBtn.addEventListener('click', () => loadFeedList(currentFeedPage - 1));
    if (nextFeedBtn) nextFeedBtn.addEventListener('click', () => loadFeedList(currentFeedPage + 1));
}
...
async function checkBlocksenseUpdates() {
    const url = "https://blocksense.network/";
    ...
    try {
        ...
        resultsDiv.innerHTML = `<div class="update-item" style="color:#e53e3e;"><div class="update-item-content">Error fetching BlockSense updates. Please try again. (${err.message})</div></div>`;
        console.error(err);
        if (lastBlocksenseChanges.length > 0) displayLastChanges();
    } finally {
